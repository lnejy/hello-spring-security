...
## 과제 2 추가 구현 (학번: 2371263 / 성명: 원재연)

### 4.2 Spring Data JPA 페이징 & 검색

- `ProductRepository`: `@Query` JPQL LIKE + `Pageable` → `Page<Product>` 반환
- `ProductService`: `getProducts(Pageable)`, `searchProducts(keyword, Pageable)`
- `ProductController`: `?page=0&size=5&keyword=` 파라미터 처리
- `products/list.html`: 페이지 네비게이션 UI, 검색 폼, 검색 조건 유지

### 4.3 상품 수정 (ROLE_ADMIN)

- `GET/POST /products/{id}/edit` 엔드포인트 추가
- 더티 체킹(Dirty Checking)으로 `save()` 없이 자동 UPDATE
- `SecurityConfig`: `/products/*/edit` → `hasRole("ADMIN")` 추가

### 4.4 비밀번호 변경

- `PasswordChangeDto`: 현재/새/확인 비밀번호 필드
- `UserService.changePassword()`: `BCryptPasswordEncoder.matches()` 검증
- `GET/POST /user/password` 엔드포인트 (로그인 사용자 전용)
